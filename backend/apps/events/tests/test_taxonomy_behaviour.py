...
@pytest.mark.django_db
def test_a_clone_copies_every_list_column_by_value(owner, make_event):  # noqa: F811
    """Not just `policies`.

    `getattr` hands back the same python list object the source instance holds,
    so without the `_LIST_FIELDS` loop the clone and the original share it and
    editing one edits the other for the life of the process.
    `test_policies_are_copied_by_value` would keep passing throughout.
    """
    source = make_event(status="draft", tags=["rooftop", "mindful"])
    Event.objects.filter(pk=source.id).update(
        highlights_included=["Chai"], highlights_excluded=["Travel"], guidelines=["Photo ID"]
    )
    ...
    ).duplicate_event(event_id=source.id, actor_id=owner.id)

    assert clone.tags == source.tags
    assert clone.tags is not source.tags
    assert clone.highlights_included is not source.highlights_included
    assert clone.highlights_excluded is not source.highlights_excluded
    assert clone.guidelines is not source.guidelines
