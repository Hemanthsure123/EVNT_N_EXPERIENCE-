 ...
 *
 * Its glyph stays INK rather than joining the violet magnifier above. This is
...
export function CompactSearchTrigger({ className }: { className?: string }) {
  const { triggerProps, preload } = useSearchOverlay();
  // `() => null`, deliberately. This is a 44px icon button on a narrow
  // viewport; a panel hung beneath it would pin to one corner with nothing to
  // align to, so it opens the CENTRED palette instead. `null` is still a stable
  // anchor identity, so the same-anchor comparison works and a second press
  // closes exactly as it does on the bar.
