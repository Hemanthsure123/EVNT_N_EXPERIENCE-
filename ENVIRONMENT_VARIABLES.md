...
| `BOOKING_IDEMPOTENCY_REPLAY_MINUTES` | Optional | `15` | `apps/booking` | How long a **paid** booking keeps answering to its `Idempotency-Key`. The checkout derives that key from the order, so unbounded it made a selection permanently unbuyable once bought — the replay returned the settled booking and Pay opened an order that had already been captured. Idempotency protects a retry of one request (seconds); a second purchase is a different intent, and elapsed time is the only thing that separates them. |
| `WAITLIST_NOTIFY_COOLDOWN_MINUTES` | Optional | `30` | `apps/events` | How long a batch told "tickets are available" gets before the next batch on the same event is told. It is the difference between a waiting list and a mailshot: one freed seat notifies three people, and without a cooldown the sweep two minutes later notifies the next three — an hour-long queue gone in half an hour, everybody told about a seat that was taken before they read the message, and nobody ever told again because a person is written to once. |
...
| `PLATFORM_FEE_BPS` | Optional | `100` | `apps/booking`, `apps/settlements` | **Basis points** of the ticket subtotal (100 = 1%). ADDED to what the customer pays, so `total = subtotal + fee`; the organizer receives the full subtotal. Replaces `PLATFORM_FEE_PER_TICKET`, which was a flat paise amount deducted from the organizer's share. Basis points rather than a float because money is integer paise everywhere and a rate stored as `0.01` is the one place a rounding error could reach a charged amount. |
| `DONATION_MAX_MINOR` | Optional | `100000` | `apps/booking` | **Minor units (paise).** Ceiling on a single donation added at checkout. The amount comes from the client, so this is what stops a checkout chip becoming an arbitrary charge. Over the cap is REFUSED, not clamped. |
